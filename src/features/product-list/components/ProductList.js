...
return <div className="min-h-screen flex items-center justify-center" ><MoonLoader
color="#36d7b7"
size={100}
/></div>
}
...
function ProductGrid({ products }) {
  ...
  return (
    <div className="bg-[#1a202c]">
      <div className="mx-auto flex justify-center min-h-screen items-center max-w-2xl px-4 py-16 sm:px-6 sm:py-10 lg:max-w-7xl lg:px-8">
        ...
        <div className="grid grid-cols-1  gap-x-6 gap-y-10 sm:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
          {products ? (
            products.map((product) => {
               ...
               return <Link to={`/product-detail/${product.id}`} key={product.id} onClick={()=> dispatch(featchSingelProductAsync({ id:product.id }))}>
