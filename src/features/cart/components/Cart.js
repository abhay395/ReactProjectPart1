...
export default function Cart() {
  ...
  if (Product.length > 0) {
    return (
      <div id="CategoryItems" class={`min-h-screen bg-[#1a202c] pb-10 pt-10`}>
        <h1 class="mb-10 text-center text-[#CCCCCC] text-2xl font-bold">
          Cart Items
        </h1>
        <div class="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0">
            ...
            {Product.map((item) => (
              <div class="justify-between lg:max-h-[150px] mb-6 rounded-lg bg-[#2d3748] p-6 shadow-md sm:flex sm:justify-start">
                <img
                  src={item.thumbnail}
                  alt="product-image"
                  class="w-full rounded-lg max-h-[150px] object-fill sm:w-40"
                />
                <div class="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                  <div class="mt-5 sm:mt-0">
                    <h2 class="text-lg font-bold text-[#CCCCCC]">
                      {item.title}
                    </h2>
                  </div>
          ...
          <div class="mt-6 h-full rounded-lg border pb-6 border-gray-700 text-gray-300 bg-[#2d3748] p-6 shadow-md md:mt-0 md:w-1/3">
            ...
            <div class="flex justify-between">
              ...
              <div class="">
                <p class="mb-1 text-lg font-bold"> {total - discount}$</p>
              </div>
            </div>
            <Link
              to="/checkout"
              class=" block mt-6 w-full text-center rounded-md bg-blue-500 py-1.5 font-medium text-blue-50 hover:bg-blue-600"
            >
              Check out
            </Link>
  ...
  return  <div id="CategoryItems" class="max-w-4xl h-screen flex items-center justify-center mx-auto px-10 py-4 ">
  <div class="flex flex-col items-center justify-center py-12">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="h-24 w-24 text-white mb-4">
      <path
        d="M4.00488 16V4H2.00488V2H5.00488C5.55717 2 6.00488 2.44772 6.00488 3V15H18.4433L20.4433 7H8.00488V5H21.7241C22.2764 5 22.7241 5.44772 22.7241 6C22.7241 6.08176 22.7141 6.16322 22.6942 6.24254L20.1942 16.2425C20.083 16.6877 19.683 17 19.2241 17H5.00488C4.4526 17 4.00488 16.5523 4.00488 16ZM6.00488 23C4.90031 23 4.00488 22.1046 4.00488 21C4.00488 19.8954 4.90031 19 6.00488 19C7.10945 19 8.00488 19.8954 8.00488 21C8.00488 22.1046 7.10945 23 6.00488 23ZM18.0049 23C16.9003 23 16.0049 22.1046 16.0049 21C16.0049 19.8954 16.9003 19 18.0049 19C19.1095 19 20.0049 19.8954 20.0049 21C20.0049 22.1046 19.1095 23 18.0049 23Z">
      </path>
    </svg>
    <p class="text-gray-400 text-lg font-semibold mb-4">Your shopping cart is empty.</p>
