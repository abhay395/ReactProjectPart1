...
function MyOrders() {
    ...
    return ( 
        ...
        <div class="flex justify-start item-start space-y-2 flex-col">
          <h1 class="text-3xl dark:text-white lg:text-4xl font-semibold leading-7 lg:leading-9 text-gray-800">Order #{item.id}</h1>
          <h1 class="text-2xl text-orange-800 dark:text-orange-600 lg:text-xl font-semibold leading-7 lg:leading-9 ">Order Status : <span className="">{item.status}</span></h1>
        ...
        <div class="mt-10 flex flex-col xl:flex-row jusitfy-center items-stretch w-full xl:space-x-8 space-y-4 md:space-y-6 xl:space-y-0">
          <div class="flex flex-col justify-start items-start w-full space-y-0 md:space-y-0 xl:space-y-0">
            {/* Summary page */}
              ...
              <div class="flex flex-col px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 dark:bg-gray-800 space-y-6">
                ...
                <div class="flex justify-center items-center w-full space-y-4 flex-col border-gray-200 border-b pb-4">
                  <div class="flex justify-between w-full">
                    ...
                    <p class="text-base dark:text-gray-300 leading-4 text-gray-600">${item.total+item.discount}</p>
                  </div>
                  <div class="flex justify-between items-center w-full">
                    <p class="text-base dark:text-white leading-4 text-gray-800">Discount </p>
                    <p class="text-base dark:text-gray-300 leading-4 text-gray-600">-${item.discount}</p>
                  </div>
                  <div class="flex justify-between items-center w-full">
                ...
                <div class="flex justify-between items-center w-full">
                  ...
                  <p class="text-base dark:text-gray-300 font-semibold leading-4 text-gray-600">${item.total+8}</p>
                </div>
              </div>
          ...
          <div class="bg-gray-50 dark:bg-gray-800 w-full xl:w-96 flex h-[400px] justify-between items-center md:items-start px-4 py-6 md:p-6 xl:p-8 flex-col">
              ...
              <div class="flex flex-col justify-start items-start flex-shrink-0">
                <div class="flex justify-center w-full md:justify-start items-center space-x-4 py-8 border-b border-gray-200">
                  ...
                  <div class="flex justify-start items-start flex-col space-y-2">
                    <p class="text-base dark:text-white font-semibold leading-4 text-left text-gray-800">{item.selectedAddress.fullName}</p>
                    <p class="text-sm dark:text-gray-300 leading-5 text-gray-600 h-1">total orders {item.items.length}</p>
                  </div>
                </div>
                ...
                <div class="flex justify-center text-gray-800 dark:text-white md:justify-start items-center space-x-4 py-4 border-b border-gray-200 w-full">
                  ...
                  <p class="cursor-pointer text-sm leading-5 ">{item.selectedAddress.email}</p>
                </div>
              </div>
              <div class="flex justify-between xl:h-full items-stretch w-full flex-col mt-6 md:mt-0">
                <div class="flex justify-center md:justify-start xl:flex-col flex-col md:space-x-6 lg:space-x-8 xl:space-x-0 space-y-4 xl:space-y-12 md:space-y-0 md:flex-row items-center md:items-start">
                  <div class="flex justify-center md:justify-start items-center md:items-start flex-col space-y-4 xl:mt-8">
                    ...
                    <p class="w-48 lg:w-full dark:text-gray-300 xl:w-48 text-center md:text-left text-sm leading-5 text-gray-600">{item.selectedAddress.city} , {item.selectedAddress.streetAddress} ,{item.selectedAddress.state} ,{item.selectedAddress.pinCode}</p>
                  </div>
